local NOTAGS = { "FX", "NOCLICK", "DECOR", "INLIMBO", "burnt", "player", "monster","white_list" }
...
local PmFireDetector = Class(function(self, inst)
    self.inst = inst
...
function PmFireDetector:SetOnFindFireFn(fn)
    self.onfindfire = fn
end

function PmFireDetector:SetOnBeginEmergencyFn(fn)
    self.onbeginemergency = fn
end

function PmFireDetector:SetOnEndEmergencyFn(fn)
    self.onendemergency = fn
end

function PmFireDetector:SetOnBeginWarningFn(fn)
    self.onbeginwarning = fn
end

function PmFireDetector:SetOnUpdateWarningFn(fn)
    self.onupdatewarning = fn
end

function PmFireDetector:SetOnEndWarningFn(fn)
...
function PmFireDetector:OnRemoveFromEntity()
...
function PmFireDetector:Activate(randomizedStartTime)
    Cancel(self.inst, self)
...
function PmFireDetector:ActivateEmergencyMode(randomizedStartTime)
    Cancel(self.inst, self)
    self.emergencyBurnt = {}
...
function PmFireDetector:IsEmergency()
    return self.emergency
end

function PmFireDetector:GetEmergencyLevel()
    return self.emergencyLevel
end

function PmFireDetector:GetMaxEmergencyLevel()
    return self.emergencyLevelMax
end

function PmFireDetector:ResetEmergencyCooldown()
    if self.emergencyBurnt ~= nil then
        local t = GetTime()
...
function PmFireDetector:RaiseEmergencyLevel(numlevels)
    if self.emergencyBurnt ~= nil then
...
function PmFireDetector:LowerEmergencyLevel(numlevels)
    if self.emergencyBurnt ~= nil then
...
function PmFireDetector:Deactivate()
...
function PmFireDetector:DetectFire()
    if self.detectTask ~= nil then
        if self.emergency then
...
function PmFireDetector:GetDebugString()
