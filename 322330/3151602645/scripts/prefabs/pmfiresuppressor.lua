...
local assets =
{
    Asset("ANIM", "anim/pmfirefighter.zip"),
...
local function TurnOff(inst, instant)
    inst.on = false
    inst.components.PmFireDetector:Deactivate()

    if not inst:HasTag("fueldepleted") then
        local randomizedStartTime = POPULATING
        inst:DoTaskInTime(0, inst.components.PmFireDetector:ActivateEmergencyMode(randomizedStartTime)) -- this can be called from onload, so make sure everything is set up first
    end
...
local function TurnOn(inst, instant)
    inst.on = true
    local isemergency = inst.components.PmFireDetector:IsEmergency()
    if not isemergency then
        local randomizedStartTime = POPULATING
        inst.components.PmFireDetector:Activate(randomizedStartTime)
...
local function OnEndEmergency(inst, level)
    if inst.on then
        --inst.components.machine:TurnOff()     --不会再自动关闭
    end
end
...
local function fn()
    ...
    inst.AnimState:SetBank("pmfirefighter")
    inst.AnimState:SetBuild("pmfirefighter")
    ...
    inst:AddComponent("PmFireDetector")
    inst.components.PmFireDetector:SetOnFindFireFn(OnFindFire)
    inst.components.PmFireDetector:SetOnBeginEmergencyFn(OnBeginEmergency)
    inst.components.PmFireDetector:SetOnEndEmergencyFn(OnEndEmergency)
    inst.components.PmFireDetector:SetOnBeginWarningFn(OnBeginWarning)
    inst.components.PmFireDetector:SetOnUpdateWarningFn(OnUpdateWarning)
    inst.components.PmFireDetector:SetOnEndWarningFn(OnEndWarning)
...
local function placer_postinit_fn(inst)
    ...
    placer2.AnimState:SetBank("pmfirefighter")
    placer2.AnimState:SetBuild("pmfirefighter")
...
return Prefab("pmfiresuppressor", fn, assets, prefabs),
