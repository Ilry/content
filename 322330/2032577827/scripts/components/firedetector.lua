local folder_name = KnownModIndex:GetModActualName("更智能的雪球机") or KnownModIndex:GetModActualName("Smarterer Ice Flingomatic")
local activate_delay_mode = GetModConfigData("activate_delay_mode", folder_name)
...
local DELAYMODETAG = {"TriggerDelayMode"}
local delayTriggerDetectRange = 5
...
local FireDetector = Class(function(self, inst)
    ...
    self.delayModeDetectRange = delayTriggerDetectRange
...
local function CheckTargetScore(target)
    ...
    if TheWorld.state.temperature < TUNING.MIN_PLANT_WITHER_TEMP or TheWorld.state.israining or target.components.witherable == nil or target.components.witherable:IsProtected() then
        return 0
...
local function EmergencyResponse(inst, self, target)
    ...
    if self.emergencyBurnt ~= nil then
        ...
        if activate_delay_mode ~= 0 then
            local x, y, z = inst.Transform:GetWorldPosition()
            local ents = TheSim:FindEntities(x, y, z, self.delayModeDetectRange, nil, NOTAGS, DELAYMODETAG)
        ...
        if activate_delay_mode ~= 0 and trapTarget ~= nil then
            if self.warningStartTime ~= nil and
                #self.emergencyBurnt >= self.emergencyLevelMax and
                GetTime() - self.warningStartTime > activate_delay_mode and 
...
local function OnDetectEmergencyTargets(inst, self)
    ...
    for i, v in ipairs(ents) do
        if not v:IsValid() then
            ...
            if TheWorld.state.temperature > TUNING.MIN_PLANT_WITHER_TEMP and not TheWorld.state.israining then
                if v.components.witherable ~= nil and not v.components.witherable:IsProtected() then 
                    if v.components.witherable:CanWither() then
                        firecount = firecount + 1
