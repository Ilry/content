...
local function defaultspawnfn(ingredient, amt, ingredientsdata, deconstructfn)
    ...
    if ingredientsdata[ingredient.signature or ingredient.type] then
        ...
        local prefab = not IsGemDictIngredient(ingredient.type) and ingredient.type or ingredient:GetFallbackPrefab()
        for n = 1, amt do
            if deconstructfn and ingredient.deconstruct then
...
local function GetFullRecipeLoot(self, recipe, ingredientsdata)
    local loots = {}

    for k, v in multiipairs(recipe.ingredients, recipe.gemdict_ingredients) do
        local amt = v.amount
        local spawnfn = v.ingredientspawnfn or defaultspawnfn
        for _, loot in ipairs(spawnfn(v, amt, ingredientsdata, function(...) return GetFullRecipeLoot(self, ...) end)) do
            table.insert(loots, loot)
        end
    end
    return loots
...
function LootDropper:GetFullRecipeLoot(recipe, ...)
    local ingredientsdata = self.inst.components.gemdict_craftinginfo and self.inst.components.gemdict_craftinginfo:GetIngredientsData() or {}
    return GetFullRecipeLoot(self, recipe, ingredientsdata)
end
...
local localdata
local _destroystructureEnv = setmetatable({
    ipairs = function(t, ...)
