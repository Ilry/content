...
local statedata =
{
	...
	},
	{
		...
		loot        = function() return math.random() <= 0.5 and {"marble"} or {"marble", "marble"} end, --随机1-2个大理石
	},
}
...
local function onworked(inst, worker, workleft)
    if workleft > 0 then
		...
		if inst.statedata.name == "tall" then
		if worker and (worker:HasTag("player") or worker.prefab == "mermguard" or worker:HasTag("willminion") or worker:HasTag("shadowminion")) then
		    inst:DoTaskInTime(0,function() --生成新的
			    local dls = SpawnPrefab("marbleshrub_short")
		        dls.Transform:SetPosition(inst.Transform:GetWorldPosition())
				dls:AddTag("event_trigger")
				dls:DoTaskInTime(180,function()  --针对暗影仆人的代码，使得仆人短时间内不挖掘新生成的小灌木
				if dls then
				    dls:RemoveTag("event_trigger")
				end
				end)
				SetupShrubShape(dls, math.random(NUM_BUILDS)) --随机贴图
		    end)
		else --专门适配下重击者
		    -- inst:DoTaskInTime(0.1,function() --特效
			    -- SpawnPrefab("sand_puff").Transform:SetPosition(inst.Transform:GetWorldPosition())
		    -- end)	
		    inst:DoTaskInTime(0.25,function() --生成新的
			    local dls = SpawnPrefab("marbleshrub_short")
		        dls.Transform:SetPosition(inst.Transform:GetWorldPosition())
				dls:AddTag("event_trigger")
				SetupShrubShape(dls, math.random(NUM_BUILDS))
		    end)
		end
		end
