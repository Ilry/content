...
local MOUSEBUTTON_DRAG = GetModConfigData("drag_button")
local MOUSEBUTTON_RESET = GetModConfigData("reset_button")
local MOUSEBUTTON_ZOOMOUT = GetModConfigData("zoomout_button")
local MOUSEBUTTON_ZOOMIN = GetModConfigData("zoomin_button")
local MIRRORFLIP = GetModConfigData("mirrorflip")

local UI_REMEMBER = GetModConfigData("remember")
...
function processUI(ui, keyortable)
    -- 判断是否是合法UI
    if not (ui and ui.SetPosition and ui.SetHAnchor and keyortable) then return end
    -- UI的参数初始化，防止UI重复处理
    ...
    elseif ui._DragZoomUIMod.acttargetkey and ui[ui._DragZoomUIMod.acttargetkey] then
        table.insert(acttargets, ui[ui._DragZoomUIMod.acttargetkey])
    elseif ui._DragZoomUIMod.acttargetparentkey and ui.parent and ui.parent[ui._DragZoomUIMod.acttargetparentkey] then
        table.insert(acttargets, ui.parent[ui._DragZoomUIMod.acttargetparentkey])
    else
    ...
    -- UI或可鼠标操作的子UI设置提示文本和代理鼠标操作
    for _, actui in ipairs(acttargets) do
        if actui.OnMouseButton then
            if ui._DragZoomUIMod.tooltip ~= "" and actui.SetTooltip then actui:SetTooltip(ui._DragZoomUIMod.tooltip) end
            if actui.SetClickable then actui:SetClickable(true) end
            local oldOnMouseButton = actui.OnMouseButton
            actui.OnMouseButton = function(_actui, button, down, x, y)
                -- 设置UI拖拽按键，拖拽中不能复位
                if button == MOUSEBUTTON_DRAG and down and not ui._DragZoomUIMod.disabledrag then -- 鼠标拖拽键按下
                    ui._DragZoomUIMod.draging = true
                    ui._DragZoomUIMod.FollowMouse(_actui) -- 开启控件的鼠标跟随
                elseif button == MOUSEBUTTON_DRAG and not ui._DragZoomUIMod.disabledrag then -- 鼠标拖拽键抬起
                    -- print("退出拖拽")
                    ui._DragZoomUIMod.draging = false
                    ui._DragZoomUIMod.StopFollowMouse(_actui) -- 停止控件的跟随
                    saveUI(ui)
                ...
                if oldOnMouseButton then return oldOnMouseButton(_actui, button, down, x, y) end
            end
        end
    end
end
