...
-- 保存所有shardindex，同时保存版本信息到本地
-- 文件内容示例:
--[[
KLEI     1 return { version=1 }
]]
...
-- 删档，会调用TheNet:DeleteCluster(slot)
...
-- 判断是否多层世界，见上方 TheSim:GetSaveFiles() 返回值格式
function ShardSaveIndex:IsSlotMultiLevel(slot)
    return self.slots[slot] == true
end

-- 获取上次游玩时间
-- 相关格式见 ShardIndex:WriteTimeFile()
-- { created=1635520175, saved=1635567486 }
...
-- 获取存档天数和季节
-- 注意该函数内对文件的操作方法，先读取.meta再读取世界存档（若前者失败）
