...
local NamePresetScreen = Class(Screen, function(self, title, confirmstr, onconfirmfn, editingpresetid, name, desc)
    ...
    self.editingpresetid = editingpresetid --要编辑的预设id
    ...
    local buttons = {
        {
            text = STRINGS.UI.CUSTOMIZATIONSCREEN.CANCEL, --取消
...
function NamePresetScreen:SavePreset()
    ...
    --存在相同名称的预设
    if KnownModIndex:GetPresetsModsData(id) then
...
function NamePresetScreen:GetID(name)
    return "world_presets/ModsPresets_CUSTOM_"..name:upper() --文件保存名字
end
