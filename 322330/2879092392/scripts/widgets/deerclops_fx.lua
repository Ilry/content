...
local Deerclops_fx = Class(Widget, function(self, owner)
    self.owner = owner
    Widget._ctor(self, "Deerclops_fx")
    self:SetClickable(false)
    self.bg = self:AddChild(Image("images/fx/gemvision_deerclops.xml", "gemvision_deerclops.tex"))
...
function Deerclops_fx:UpdateVisor(data)
    local hat = self.owner.replica.inventory:GetEquippedItem(EQUIPSLOTS.HEAD)
    if hat and hat:HasTag("deerclops_fx") then
        self:Show()
    else
        self:Hide()
    end
