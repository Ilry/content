local assets = {
    Asset("ANIM", "anim/why_orangegem_seed.zip"),
    Asset("ANIM", "anim/why_orangegem_formation.zip"),
    Asset("ATLAS", "images/map_icons/why_orangegem_formation.xml"),
    Asset("IMAGE", "images/map_icons/why_orangegem_formation.tex"),
    Asset("ATLAS", "images/inventoryimages/why_orangegem_seed.xml"),
    Asset("IMAGE", "images/inventoryimages/why_orangegem_seed.tex"),
    Asset("ATLAS_BUILD", "images/inventoryimages/why_orangegem_seed.xml",256), }
local function ondeploy(inst, pt, deployer)
    local sapling = SpawnPrefab("why_orangegem_formation")
    sapling.components.growable:StartGrowing()
    sapling.Transform:SetPosition(pt:Get())
    sapling.SoundEmitter:PlaySound("dontstarve/wilson/plant_tree")
    inst:Remove()
end
local function orangegemseed()
    ...
    inst.AnimState:SetBank("why_orangegem_seed")
    inst.AnimState:SetBuild("why_orangegem_seed")
    ...
    inst.components.inventoryitem.imagename = "why_orangegem_seed"
    inst.components.inventoryitem.atlasname = "images/inventoryimages/why_orangegem_seed.xml"
...
local statedata = {
    { name = "short",
      idleanim = "why_orangegem_sapling",
        --hitanim     = "hit_short",
    ...
    { name = "normal",
      ...
      workleft = 6,
      loot = function()
          return math.random() < 0.5 and { "orangegem" } or { "orangegem", "ancientdreams_gemshard" }
      end, },
    { name = "tall",
      idleanim = "full",
      hitanim = "full_hit",
      ...
      workleft = 8,
      loot = function()
          return { "orangegem", "ancientdreams_gemshard", "why_orangegem_overgrown" }
      end, }, }
...
local GROWTH_STAGES = {
    ...
    { time = function(inst)
        return GetRandomWithVariance(2800, 960)
    end, 
      fn = function(inst)
          SetGrowth(inst)
      end,
...
local function MakeShrub(name, growthstage)
    ...
    local function fn()
        local inst = CreateEntity()
        inst:SetPrefabName("why_orangegem_formation")
        inst.entity:AddTransform()
        inst.entity:AddAnimState()
        inst.entity:AddSoundEmitter()
        inst.entity:AddNetwork()
        ...
        inst.MiniMapEntity:SetIcon("why_orangegem_formation.tex")
        inst.MiniMapEntity:SetPriority(5.5)
        inst.AnimState:SetBank("why_orangegem_formation")
        inst.AnimState:SetBuild("why_orangegem_formation")
        inst.AnimState:PlayAnimation("why_orangegem_sapling")
...
return Prefab("why_orangegem_seed", orangegemseed, assets),
MakePlacer("why_orangegem_seed_placer", "why_orangegem_formation", "why_orangegem_formation", "why_orangegem_sapling"),
MakeShrub("why_orangegem_formation"),
MakeShrub("why_orangegem_formation_short", 1),
MakeShrub("why_orangegem_formation_normal", 2),
MakeShrub("why_orangegem_formation_tall", 3)