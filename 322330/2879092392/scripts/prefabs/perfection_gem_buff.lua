...
-----------------------------------------Borealis Length below this line-----------------------------------------

local TORCH_BUFF_DURATION = TUNING.TOTAL_DAY_TIME

local function t_OnAttached(inst, target)
    if target ~= nil and target:IsValid() then
        inst.entity:SetParent(target.entity)
        inst.Transform:SetPosition(0, 0, 0) --in case of loading
        inst:ListenForEvent("death", function()
        ...
        if inst.components.timer then
            inst.components.timer:StartTimer("perfectionbuff_timer", TORCH_BUFF_DURATION)
        end
    end
end

local function t_OnDetached(inst, target)
    if target ~= nil and target:IsValid() then
        if target.components.talker ~= nil then
            if TUNING.WHY_LANGUAGE == "spanish" then
                target.components.talker:Say("Ya no es perfecto.")
...
local function t_OnExtended(inst)
    if inst.components.timer and
        inst.components.timer:TimerExists("perfectionbuff_timer") then

        inst.components.timer:SetTimeLeft("perfectionbuff_timer", TORCH_BUFF_DURATION)
        
    end
end

local function torchbuff_fn()
    local inst = CreateEntity()
    ...
    inst:AddComponent("debuff")
    inst.components.debuff:SetAttachedFn(t_OnAttached)--设置附加Buff时执行的函数
    inst.components.debuff:SetDetachedFn(t_OnDetached)--设置解除buff时执行的函数
    inst.components.debuff:SetExtendedFn(t_OnExtended)--设置延长buff时执行的函数
    inst.components.debuff.keepondespawn = true
...
return Prefab("perfection_gem_buff", fn),
       Prefab("whytorch_perfection_buff", torchbuff_fn)
