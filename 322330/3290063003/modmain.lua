GLOBAL.TOWNPORTALTALISMANCOST = GetModConfigData("TRANSMISSIONTOWERCONSUM")
...
local function OnActivate(inst, doer)
	if doer:HasTag("player") then
		...
		if doer.components.sanity ~= nil then
			doer.components.sanity:DoDelta(-GetModConfigData("TRANSMISSIONTOWERCONSUM2"))  
		end
	end
end
local Prefab_Post = {}

local function Stackable_Post(inst, maxsize)
	if not inst.components.stackable then
		inst:AddComponent("stackable")
	end

	inst.components.stackable.maxsize = maxsize
end
local function Armor_Post(inst, amount, absorb_percent, repairable_item, SetCondition)
	if repairable_item then
		inst:AddTag("repairable")
		inst.repairable_item = repairable_item
	end

	local armor = inst.components.armor
	if not TheWorld.ismastersim or not armor then
		return
	end

	if amount then
		armor.condition = amount
		armor.maxcondition = amount
	end

	if absorb_percent then
	...
	if SetCondition then
		armor.SetCondition = function(self, amount)
			...
			if self.condition <= 0 then
				self.condition = 0
				SetCondition.turn_off(self)
			else
				SetCondition.turn_on(self)
			end
		end
	end
end

local function Weapon_Post(inst, damage, onattackFn)
	...
	if onattackFn then
		weapon:SetOnAttack(onattackFn)
	end
end

local function Finiteuses_Post(inst, uses, repairable_item, OnFinished, doesnotstartfull)
	if repairable_item then
		inst:AddTag("repairable")
		inst.repairable_item = repairable_item
	end
	...
	if not TheWorld.ismastersim or not finiteuses then
		return
	end
	...
	if doesnotstartfull then
		finiteuses:SetDoesNotStartFull(true)
	end
end

local function Fueled_Post(inst, InitializeFuelLevel, TakeFuelItem, DepletedFn, repairable_item)
	...
	if repairable_item then
		...
		if TheWorld.ismastersim and fueled then
			fueled.accepting = false  
		end
	end

	if not TheWorld.ismastersim or not fueled then
		return
	end

	if InitializeFuelLevel then
		fueled:InitializeFuelLevel(InitializeFuelLevel)
	end

	if TakeFuelItem then
		fueled.TakeFuelItem = TakeFuelItem
	end

	if DepletedFn then
		inst.components.fueled:SetDepletedFn(DepletedFn)
	end
end
...
local TravelScreen = require "screens/travelscreen2"
...
function Teleporter:TravelTeleport(obj, userid)
	if self.traveltarget[userid] ~= nil then
		...
		local offset = self.traveltarget[userid].components.teleporter ~= nil and self.traveltarget[userid].components.teleporter.offset or 0
...
local need_num = GetModConfigData("TRANSMISSIONTOWERCONSUM")
...
Prefab_Post.townportaltalisman = {  
	fn = function (inst)
		if inst.components.teleporter then
			inst.components.teleporter.onActivate = OnActivate
		end
...
Prefab_Post.townportaltalisman.stackable = {
		maxsize = GLOBAL.TUNING.STACK_SIZE_SMALLITEM
}
for item, post in pairs(Prefab_Post) do
	AddPrefabPostInit(item, function(inst)

		if post.armor then
			Armor_Post(inst, post.armor.amount, post.armor.absorb_percent, post.armor.repairable_item, post.armor.SetCondition)
		end

		if post.finiteuses then
			Finiteuses_Post(inst, post.finiteuses.uses, post.finiteuses.repairable_item, post.finiteuses.OnFinished, post.finiteuses.doesnotstartfull)
		end

		if post.fueled then
			Fueled_Post(inst, post.fueled.InitializeFuelLevel, post.fueled.TakeFuelItem, post.fueled.DepletedFn, post.fueled.repairable_item)
		end
		...
		if post.stackable then
			Stackable_Post(inst, post.stackable.maxsize)
		end


		if post.weapon then
			Weapon_Post(inst, post.weapon.damage, post.weapon.onattackFn)
		end
