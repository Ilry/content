...
local id = "COMBINEWEAPON" --必须大写，动作会被加入到ACTIONS表中，key就是id。
...
local fn = function(act) -- 动作触发的函数。传入的是一个BufferedAction对象。可以通过它直接调用动作的执行者，目标，具体的动作内容等等，详情请查看bufferedaction.lua文件，也可以参考actions.lua里各个action的fn。
    ...
    --    return
    --end
	--if act.doer:HasTag("tentaclemedal") then
	--	return 
	--end
	...
	if target.components.finiteuses ~= nil and inst.components.finiteuses ~= nil then
		if TUNING.RANDCLIMITED == false then
			target.components.finiteuses:SetPercent(math.min(1,target.components.finiteuses:GetPercent()+inst.components.finiteuses:GetPercent()))
			target:PushEvent("percentusedchange", { percent = target.components.finiteuses:GetPercent() })
		else
			target.components.finiteuses.current = math.min(newlimit * target.components.finiteuses.total,target.components.finiteuses.current+inst.components.finiteuses.current)
			target:PushEvent("percentusedchange", { percent = target.components.finiteuses:GetPercent() })
		end
		if TUNING.RANGCCOFINITEUSES ~= 0 then
			target.components.finiteuses.combinetime = inst.components.finiteuses.combinetime + target.components.finiteuses.combinetime
			if target.components.finiteuses.combinetime >= TUNING.RANGCCOFINITEUSES then
...
local component = "finiteuses" -- 设置动作绑定的组件
local testfn = function(inst, doer, target, actions, right) -- 设置动作的检测函数，如果满足条件，就向人物的动作可执行表中加入某个动作。right表示是否是右键动作。
	if target.prefab == inst.prefab
	and not (doer:HasTag("tentaclemedal") and inst.prefab == "medal_tentaclespike")
	then
		table.insert(actions, ACTIONS.COMBINEWEAPON)
	end
end
...
AddStategraphActionHandler("wilson",ActionHandler(ACTIONS.COMBINEWEAPON, state))
AddStategraphActionHandler("wilson_client",ActionHandler(ACTIONS.COMBINEWEAPON,state))
