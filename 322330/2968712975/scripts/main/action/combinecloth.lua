...
local id = "COMBINECLOTH" --必须大写，动作会被加入到ACTIONS表中，key就是id。
...
local fn = function(act) -- 动作触发的函数。传入的是一个BufferedAction对象。可以通过它直接调用动作的执行者，目标，具体的动作内容等等，详情请查看bufferedaction.lua文件，也可以参考actions.lua里各个action的fn。
	...
	if target.components.fueled ~= nil and inst.components.fueled ~= nil then
		if TUNING.RANDCLIMITED == false then
			target.components.fueled:SetPercent(math.min(1,target.components.fueled:GetPercent()+inst.components.fueled:GetPercent()))
			target:PushEvent("percentusedchange", { percent = target.components.fueled:GetPercent() })
		else
			target.components.fueled.currentfuel = math.min(newlimit * target.components.fueled.maxfuel,target.components.fueled.currentfuel + inst.components.fueled.currentfuel)
			target:PushEvent("percentusedchange", { percent = target.components.fueled:GetPercent() })
			--target.components.fueled.maxfuel = math.max(target.components.fueled.currentfuel,target.components.fueled.maxfuel)
		end
		if TUNING.RANGCCOFINITEUSES ~= 0 then
			target.components.fueled.combinetime = inst.components.fueled.combinetime + target.components.fueled.combinetime
			if target.components.fueled.combinetime >= TUNING.RANGCCOFINITEUSES then
...
local component = "fueled" -- 设置动作绑定的组件
local testfn = function(inst, doer, target, actions, right) -- 设置动作的检测函数，如果满足条件，就向人物的动作可执行表中加入某个动作。right表示是否是右键动作。
	...
	then
		table.insert(actions, ACTIONS.COMBINECLOTH)
	end
end
...
AddStategraphActionHandler("wilson",ActionHandler(ACTIONS.COMBINECLOTH, state))
AddStategraphActionHandler("wilson_client",ActionHandler(ACTIONS.COMBINECLOTH,state))
