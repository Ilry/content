...
local function IsWorkshopMod(name) return name:find(modutils.workshopprefix) end
modutils = {
  ...
  enabled = function(name)
    return KnownModIndex:IsModEnabledAny(name) or KnownModIndex:IsModEnabled(modutils.translate(name)) or
        KnownModIndex:IsModForceEnabled(modutils.translate(name))
        or KnownModIndex:IsModForceEnabled(name)
  end
}
...
function specialGameModeDetector()
  ...
  return gameMode == "lavaarena" and "forge"
      or gameMode == "quagmire" and "gorge"
      or "normal"
end
...
function PreloadAsset(assettype, name, format)
  if not format then
    format = nil
  ...
  theplayer = function(fn, ...)
    ...
    utils.player(function(p)
      if not ThePlayer then
        AddEventListener(p, "playeractivated", function() fn(p, unpack(param)) end)
        return
      end
