local prefab_id = "lol_wp_s10_blastingwand"
local assets_id = "lol_wp_s10_blastingwand"
...
local function onattack(inst,attacker,target)
    if target and target.components.burnable then
        ...
        if target.taskperiod_lol_wp_s10_blastingwand_fire == nil then
            target.taskperiod_lol_wp_s10_blastingwand_fire = target:DoPeriodicTask(TUNING.MOD_LOL_WP.SUNFIREAEGIS.SKILL_FIRETOUCH.BURN_PERIOD,function ()
                if LOLWP_S:checkAlive(target) then
                    target.components.health:DoDelta(-TUNING.MOD_LOL_WP.SUNFIREAEGIS.SKILL_FIRETOUCH.BURN_DMG)
                end
            end)
            target:DoTaskInTime(TUNING.MOD_LOL_WP.SUNFIREAEGIS.SKILL_FIRETOUCH.BURN_LAST,function()
                if target then
                    if target.components.burnable then
                        target.components.burnable:KillFX()
                    end
                    if target.taskperiod_lol_wp_s10_blastingwand_fire then
                        target.taskperiod_lol_wp_s10_blastingwand_fire:Cancel()
                        target.taskperiod_lol_wp_s10_blastingwand_fire = nil
                    end
                end
            end)
        end
