...
local prefab_id = "lol_wp_s7_doranblade"
...
local function onattack(inst,attacker,target)
    if not target:HasTag("structure") and not target:HasTag("wall") and target.components.combat and target.components.health then
        if LANS:checkAlive(attacker) then
            attacker.components.health:DoDelta(TUNING.MOD_LOL_WP.DORANBLADE.DRAIN)
        end
    end
end
...
local function fn()
    ...
    inst.components.weapon:SetDamage(TUNING.MOD_LOL_WP.DORANBLADE.DMG)
    inst.components.weapon:SetOnAttack(onattack)

    inst:AddComponent("finiteuses")
    inst.components.finiteuses:SetMaxUses(TUNING.MOD_LOL_WP.DORANBLADE.FINITEUSE)
    inst.components.finiteuses:SetUses(TUNING.MOD_LOL_WP.DORANBLADE.FINITEUSE)
