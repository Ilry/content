...
local prefab_id = 'lol_wp_s11_mejaisoulstealer'
...
local function onequip(inst, owner)
    if not inst.lol_wp_s11_mejaisoulstealer_isequip then
        ...
        inst.lol_wp_s11_mejaisoulstealer_isequip = true

        if owner.components.lol_wp_player_dmg_adder then
            local num = inst.components.lol_wp_s11_mejaisoulstealer_num and inst.components.lol_wp_s11_mejaisoulstealer_num:GetVal() or 0
            local dmg = num * db.SKILL_HONOR.EXTRA_PLANAR_DMG_PER_STACK + db.PLANAR_DMG
...
local function onunequip(inst, owner)
    if inst.lol_wp_s11_mejaisoulstealer_isequip then
        owner.AnimState:ClearOverrideSymbol("swap_body")
        if inst.components.lol_wp_s11_mejaisoulstealer_num then
            inst.components.lol_wp_s11_mejaisoulstealer_num:WhenUnequip(inst,owner)
        end
        inst.lol_wp_s11_mejaisoulstealer_isequip = false
...
local function fn()
    ...
    inst:AddComponent('lol_wp_s11_mejaisoulstealer_num')
