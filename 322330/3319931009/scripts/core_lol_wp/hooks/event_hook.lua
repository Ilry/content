...
local allevents = {
    ...
    ['knockback'] = {
        ...
        lol_wp_s20_wardenmail_armor_and_frozenheart_armor = { -- 守望者铠甲 冰霜之心 击飞效果
            fn = function (equips_map,player)
                if equips_map['lol_wp_s20_wardenmail_armor'] or equips_map['lol_wp_s20_frozenheart_armor'] or LOLWP_U:getEquipInEyeStone(player,'lol_wp_s20_frozenheart_armor') then
                    return true
                end
                return false
            end,
    ...
    ['knockedout'] = {
        lol_wp_s17_liandry = {
            fn = function (equips_map, player)
                ...
                if player:HasDebuff('buff_lol_wp_s16_potion_corruption') then
                    return true
                end
                return false
            end
    ...
    ['freeze'] = {
        lol_wp_s19_fimbulwinter_armor_upgrade = { -- 凛冬之临
            fn = function (equips_map,player)
    ...
    ['feetslipped'] = {
        lol_wp_s20_frozenheart_armor = {
            fn = function (equips_map, player)
                if equips_map['lol_wp_s20_frozenheart_armor'] or LOLWP_U:getEquipInEyeStone(player,'lol_wp_s20_frozenheart_armor') then
                    return true
                end
                return false
            end
    ...
    ['pinned'] = {
        set1 = {
            -- 能不能让金身，王冕的护盾，恶魔之拥+霸王血铠的套装效果，可以免疫钢羊的鼻涕
            fn = function (equips_map, player)
                if equips_map['lol_wp_demonicembracehat'] and equips_map['lol_wp_overlordbloodarmor'] then
                    return true
                elseif player.lol_wp_s15_crown_of_the_shattered_queen_invincible then
                    return true
                elseif player.lol_wp_s15_zhonya_invincible then
                    return true
                end
                return false
            end
