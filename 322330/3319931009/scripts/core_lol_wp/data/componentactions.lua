    ...
    -- 日炎圣盾 切换形态
    {
        ...
        fn = function (act)
            ...
            if sunfire and player and sunfire:IsValid() and player:IsValid() and sunfire.components.armor and player.components.inventory then
                ...
                local skin_build = sunfire:GetSkinBuild() or ''
                local start_str, end_str = string.find(skin_build, '_skin_')
                local suffix = nil
                if start_str then
                    suffix = string.sub(skin_build, end_str + 1)
                end
                local prod = SpawnPrefab(tf_to,suffix ~= nil and tf_to..'_skin_'..suffix or nil)
            ...
            if player and weapon and player:IsValid() and weapon:IsValid() then
                if weapon.components.lol_wp_s12_eclipse_leap_laser then
                    if player.SoundEmitter then
                        player.SoundEmitter:PlaySound("dontstarve/creatures/deerclops/laser")
                    end
    ...
    -- 女神泪升级的武器,满层后,给物品升级
    {
        ...
        fn = function (act)
            ...
            if doer and item and wp and doer:IsValid() and item:IsValid() and wp:IsValid() then
                ...
                if upgrade_to then
                    local skin_build = wp:GetSkinBuild() or ''
                    local start_str, end_str = string.find(skin_build, '_skin_')
                    local suffix = nil
                    if start_str then
                        suffix = string.sub(skin_build, end_str + 1)
                    end
                    local new_wp = SpawnPrefab(upgrade_to,suffix ~= nil and upgrade_to..'_skin_'..suffix or nil)
                    wp:Remove()
                    if doer.components.inventory then
