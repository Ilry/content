...
-- lol_wp_pedia_items_scroll expects a table of pre-constructed items to be handed in as the "items" param OR
...
local lol_wp_pedia_items_scroll = Class(Widget, function(self, items, listwidth, listheight, itemheight, itempadding, updatefn, widgetstoupdate, widgetXOffset, always_show_static, starting_offset, yInit, bar_width_scale_factor, bar_height_scale_factor, scrollbar_style)
...
function lol_wp_pedia_items_scroll:DebugDraw_AddSection(dbui, panel)
    lol_wp_pedia_items_scroll._base.DebugDraw_AddSection(self, dbui, panel)

    dbui.Spacing()
    dbui.Text("lol_wp_pedia_items_scroll")
    dbui.Indent() do
...
function lol_wp_pedia_items_scroll:OnControl(control, down, force)
    if lol_wp_pedia_items_scroll._base.OnControl(self, control, down) then return true end
...
function lol_wp_pedia_items_scroll:Scroll(amt, movemarker)
...
function lol_wp_pedia_items_scroll:RefreshView(movemarker)
    local showing = false
...
function lol_wp_pedia_items_scroll:LayOutStaticWidgets(yInitial, skipFixUp, focusChildren)
...
function lol_wp_pedia_items_scroll:GetNearestStep()
    local marker = self.position_marker:GetPosition()
    return math.floor((marker.y / self.step_size) + 0.5)
end

function lol_wp_pedia_items_scroll:GetHierarchicalScale()
...
function lol_wp_pedia_items_scroll:DoDragScroll()
...
function lol_wp_pedia_items_scroll:MoveMarkerToNearestStep()
...
function lol_wp_pedia_items_scroll:SetScrollPerClick(amt)
    scroll_per_click = amt
end

function lol_wp_pedia_items_scroll:SetScrollPerPage(amt)
    scroll_per_page = amt
end

function lol_wp_pedia_items_scroll:RecalculateStepSize()
...
function lol_wp_pedia_items_scroll:SetListItemPadding(pad)
    self.item_padding = pad
    self:RecalculateStepSize()
    self:RefreshView()
end

function lol_wp_pedia_items_scroll:SetListItemHeight(ht)
    self.item_height = ht
    self:RecalculateStepSize()
    self:RefreshView()
end
...
function lol_wp_pedia_items_scroll:SetList(list, keepitems, scrollto, keeprelativefocusindex)
...
function lol_wp_pedia_items_scroll:AddItem(item, before_widget)
...
function lol_wp_pedia_items_scroll:RemoveItem(item)
    local index = -1
    for i,v in ipairs(self.items) do
        if v == item then
            index = i
...
function lol_wp_pedia_items_scroll:Clear()
    if self.updatefn == nil and self.static_widgets == nil then
        for i, v in ipairs(self.items) do
            v:Kill()
        end
...
function lol_wp_pedia_items_scroll:GetNumberOfItems()
    return #self.items
end

function lol_wp_pedia_items_scroll:OnGainFocus()
    lol_wp_pedia_items_scroll._base.OnGainFocus(self)
...
function lol_wp_pedia_items_scroll:OnLoseFocus()
    lol_wp_pedia_items_scroll._base.OnLoseFocus(self)
...
function lol_wp_pedia_items_scroll:SetFocus()
...
function lol_wp_pedia_items_scroll:DoFocusHookups()
    -- Static table of widgets that we show and hide
...
function lol_wp_pedia_items_scroll:OnFocusMove(dir, down)
    if lol_wp_pedia_items_scroll._base.OnFocusMove(self,dir,down) then return true end
...
function lol_wp_pedia_items_scroll:GetHelpText()
    local controller_id = TheInput:GetControllerID()
    local t = {}
    if self.scroll_bar and self.scroll_bar:IsVisible() then
        table.insert(t, TheInput:GetLocalizedControl(controller_id, CONTROL_SCROLLBACK, false, false).."/"..TheInput:GetLocalizedControl(controller_id, CONTROL_SCROLLFWD, false, false).. " " .. STRINGS.UI.HELP.SCROLL)
...
function lol_wp_pedia_items_scroll:IsAtEnd()
    return self.view_offset == self.max_step
end

function lol_wp_pedia_items_scroll:ScrollToEnd()
    if self.scroll_bar and self.scroll_bar:IsVisible() then
        self:Scroll(self:GetNumberOfItems(), true)
    end
end

return lol_wp_pedia_items_scroll
