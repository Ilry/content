...
-- lol_wp_pedia_scrollablelist expects a table of pre-constructed items to be handed in as the "items" param OR
...
local lol_wp_pedia_scrollablelist = Class(Widget, function(self, groups_order, listwidth, listheight, itemheight, itempadding, updatefn, widgetstoupdate, widgetXOffset, always_show_static, starting_offset, yInit, bar_width_scale_factor, bar_height_scale_factor, scrollbar_style,scroll_page_member)
    Widget._ctor(self, "lol_wp_pedia_scrollablelist")
    ...
    self.scroll_page_member = scroll_page_member
    self.items = self.scroll_page_member[groups_order[1]]
    if updatefn and widgetstoupdate then
        self.updatefn = updatefn
        self.static_widgets = widgetstoupdate
    else
        for i,name in ipairs(groups_order) do
            for _,v in ipairs(self.scroll_page_member[name]) do
                self:AddChild(v)
                if i ~= 1 then
                    v:Hide()
                end
...
function lol_wp_pedia_scrollablelist:DebugDraw_AddSection(dbui, panel)
    lol_wp_pedia_scrollablelist._base.DebugDraw_AddSection(self, dbui, panel)

    dbui.Spacing()
    dbui.Text("lol_wp_pedia_scrollablelist")
    dbui.Indent() do
...
function lol_wp_pedia_scrollablelist:OnControl(control, down, force)
    if lol_wp_pedia_scrollablelist._base.OnControl(self, control, down) then return true end
...
function lol_wp_pedia_scrollablelist:Scroll(amt, movemarker)
...
function lol_wp_pedia_scrollablelist:RefreshView(movemarker)
    local showing = false
...
function lol_wp_pedia_scrollablelist:LayOutStaticWidgets(yInitial, skipFixUp, focusChildren)
...
function lol_wp_pedia_scrollablelist:GetNearestStep()
    local marker = self.position_marker:GetPosition()
    return math.floor((marker.y / self.step_size) + 0.5)
end

function lol_wp_pedia_scrollablelist:GetHierarchicalScale()
...
function lol_wp_pedia_scrollablelist:DoDragScroll()
...
function lol_wp_pedia_scrollablelist:MoveMarkerToNearestStep()
...
function lol_wp_pedia_scrollablelist:SetScrollPerClick(amt)
    scroll_per_click = amt
end

function lol_wp_pedia_scrollablelist:SetScrollPerPage(amt)
    scroll_per_page = amt
end

function lol_wp_pedia_scrollablelist:RecalculateStepSize()
...
function lol_wp_pedia_scrollablelist:SetListItemPadding(pad)
    self.item_padding = pad
    self:RecalculateStepSize()
    self:RefreshView()
end

function lol_wp_pedia_scrollablelist:SetListItemHeight(ht)
    self.item_height = ht
    self:RecalculateStepSize()
    self:RefreshView()
end
...
function lol_wp_pedia_scrollablelist:SetList(list, keepitems, scrollto, keeprelativefocusindex,group_index)
    ...
    -- ThePlayer:DoTaskInTime(0,function ()
        self.items = self.scroll_page_member[self.groups_order[group_index]]
...
function lol_wp_pedia_scrollablelist:AddItem(item, before_widget)
...
function lol_wp_pedia_scrollablelist:RemoveItem(item)
    local index = -1
    for i,v in ipairs(self.items) do
        if v == item then
            index = i
...
function lol_wp_pedia_scrollablelist:Clear()
    if self.updatefn == nil and self.static_widgets == nil then
        for i, v in ipairs(self.items) do
            v:Kill()
        end
...
function lol_wp_pedia_scrollablelist:GetNumberOfItems()
    return #self.items
end

function lol_wp_pedia_scrollablelist:OnGainFocus()
    lol_wp_pedia_scrollablelist._base.OnGainFocus(self)
...
function lol_wp_pedia_scrollablelist:OnLoseFocus()
    lol_wp_pedia_scrollablelist._base.OnLoseFocus(self)
...
function lol_wp_pedia_scrollablelist:SetFocus()
...
function lol_wp_pedia_scrollablelist:DoFocusHookups()
    -- Static table of widgets that we show and hide
...
function lol_wp_pedia_scrollablelist:OnFocusMove(dir, down)
    if lol_wp_pedia_scrollablelist._base.OnFocusMove(self,dir,down) then return true end
...
function lol_wp_pedia_scrollablelist:GetHelpText()
    local controller_id = TheInput:GetControllerID()
    local t = {}
    if self.scroll_bar and self.scroll_bar:IsVisible() then
        table.insert(t, TheInput:GetLocalizedControl(controller_id, CONTROL_SCROLLBACK, false, false).."/"..TheInput:GetLocalizedControl(controller_id, CONTROL_SCROLLFWD, false, false).. " " .. STRINGS.UI.HELP.SCROLL)
...
function lol_wp_pedia_scrollablelist:IsAtEnd()
    return self.view_offset == self.max_step
end

function lol_wp_pedia_scrollablelist:ScrollToEnd()
    if self.scroll_bar and self.scroll_bar:IsVisible() then
        self:Scroll(self:GetNumberOfItems(), true)
    end
end

return lol_wp_pedia_scrollablelist
