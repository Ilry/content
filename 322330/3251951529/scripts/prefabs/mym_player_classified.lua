...
local function UpdateAnimOverrideSanity(parent)
    local isinsane = parent.replica.sanity:IsInsanityMode() and
        (parent.replica.sanity:GetPercentNetworked() <= (parent:HasTag("dappereffects") and TUNING.DAPPER_BEARDLING_SANITY or TUNING.BEARDLING_SANITY))
...
local function SetTemperature(inst, temperature)
    if temperature >= max_precision_temp then
        inst.currenttemperaturedata:set(pivot + max_precision_temp * precision_factor +
            math.floor((temperature - max_precision_temp) * coarse_factor + .5))
    elseif temperature <= min_precision_temp then
        inst.currenttemperaturedata:set(pivot + min_precision_temp * precision_factor +
            math.floor((temperature - min_precision_temp) * coarse_factor + .5))
    else
...
local function DeserializeTemperature(inst)
    if inst.currenttemperaturedata:value() >= pivot + max_precision_temp * precision_factor then
        inst.currenttemperature = (inst.currenttemperaturedata:value() - pivot - max_precision_temp * precision_factor) /
            coarse_factor + max_precision_temp
    elseif inst.currenttemperaturedata:value() <= pivot + min_precision_temp * precision_factor then
        inst.currenttemperature = (inst.currenttemperaturedata:value() - pivot - min_precision_temp * precision_factor) /
            coarse_factor + min_precision_temp
    else
...
local function OnAttackedPulseEvent(inst)
    if inst._parent ~= nil then
        inst._parent:PushEvent("attacked",
            { isattackedbydanger = inst.isattackedbydanger:value(), redirected = inst.isattackredirected:value() })
    end
end
...
local function BufferBuild(inst, recipename)
    ...
    local inventory = inst._parent ~= nil and inst._parent.replica.inventory ~= nil and
        inst._parent.replica.inventory.classified or nil
...
local function OnPausePredictionFramesDirty(inst)
    if inst._parent ~= nil and inst._parent.HUD ~= nil then
        ...
        inst._pausepredictiontask = inst.pausepredictionframes:value() > 0 and
            inst:DoTaskInTime(inst.pausepredictionframes:value() * FRAMES, CancelPausePrediction) or nil
...
local function OnActionMeterDirty(inst)
    if inst._parent ~= nil and inst._parent.HUD ~= nil then
        if inst.actionmeter:value() < 2 then
            ...
            inst._parent.HUD:ShowRingMeter(inst._parent:GetPosition(), inst.actionmetertime:value() * .1,
                (inst.actionmeter:value() - 2) * .1)
        end
    end
end
...
local function OnPlayerCameraDirty(inst)
    if inst._parent ~= nil and inst._parent.HUD ~= nil then
        ...
        elseif inst._prevcameradistance ~= nil then
            TheCamera:SetDistance(inst.cameradistance:value() > 0 and inst.cameradistance:value() or
                inst._prevcameradistance)
...
function fns.OnInitialDirtyStates(inst)
    if not TheWorld.ismastersim then
        ...
        if inst._parent ~= nil then
            inst._oldhealthpercent = inst.maxhealth:value() > 0 and inst.currenthealth:value() / inst.maxhealth:value() or
                0
            inst._oldhungerpercent = inst.maxhunger:value() > 0 and inst.currenthunger:value() / inst.maxhunger:value() or
                0
            inst._oldsanitypercent = inst.maxsanity:value() > 0 and inst.currentsanity:value() / inst.maxsanity:value() or
                0
...
local function fn()
    ...
    inst._pausepredictiontask = nil
    inst.pausepredictionframes = net_tinybyte(inst.GUID, "playercontroller.pausepredictionframes",
        "pausepredictionframesdirty")
    ...
    for k, v in pairs(AllRecipes) do
        if IsRecipeValid(v.name) then
            -- ### 阻止队友recipes和bufferedbuilds网络变量的创建
            inst.recipes[k] = DEFAULT_BOOL_VAL
            inst.bufferedbuilds[k] = DEFAULT_BOOL_VAL
        end
    end
...
return Prefab("mym_player_classified", fn) --###
