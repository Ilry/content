...
inst.fx.Follower:FollowSymbol(inst.GUID, "headbase", 0, 0, -4, true, nil, 0);
-- 颜色乘积，r、g、b、alpha，可以调整颜色和不透明度
inst.fx.AnimState:SetMultColour(1, 1, 1, 1);
-- 颜色相加，也是r、g、b、alpha，可以调整颜色和不透明度
inst.fx.AnimState:SetAddColour(0, 0, 0, 0);
-- 动画播放速度
inst.fx.AnimState:SetDeltaTimeMultiplier(1);
-- 缩放
inst.fx.AnimState:SetScale(1, 1, 1);
-- 应该叫后期处理吧
inst.fx.AnimState:SetBloomEffectHandle("shaders/anim_bloom_ghost.ksh");
inst.fx.AnimState:SetBloomEffectHandle("shaders/anim.ksh");
-- 夜晚可见度
inst.fx.AnimState:SetLightOverride(1);
-- 动画层级，FollowSymbol倒数第三个参数为false时起效
inst.fx.AnimState:SetSortOrder(1);
-- 图层优先级
inst.fx.AnimState:SetFinalOffset(0);

inst.fx.AnimState:SetLayer(LAYER_BACKGROUND);
