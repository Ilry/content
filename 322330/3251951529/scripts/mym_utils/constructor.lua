...
function FN.OnHatEquip(inst, owner, fname)
...
function FN.MakeHatFn(bank, build, data)
    local playAnim = Utils.GetVal(data, "playAnim", "anim")                 --默认播放动画，建议名为anim，并且为在地面上的静止动画
    local coomonInit = Utils.GetVal(data, "coomonInit")
    ...
    local onEquipFn = Utils.GetVal(data, "onEquipFn", function(inst, owner)
        FN.OnHatEquip(inst, owner, build)
        if onEquipPostFn then onEquipPostFn(inst, owner) end
    end)                                                                                                                  --装备时
    local onOnUnequipFn = Utils.GetVal(data, "onOnUnequipFn", Utils.MergeFn(FN.OnHatUnequip, onUnequipPostFn))            --卸下时
