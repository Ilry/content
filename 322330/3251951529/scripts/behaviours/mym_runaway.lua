--- 改动：
--- 让safe_dist支持函数，safe_dist(inst,hunter)
MYM_RunAway = Class(BehaviourNode,
    function(self, inst, hunterparams, see_dist, safe_dist, fn, runhome, fix_overhang, walk_instead, safe_point_fn)
        BehaviourNode._ctor(self, "MYM_RunAway")
        ...
        self.shouldrunfn = fn
        self.fix_overhang =
            fix_overhang -- this will put the point check back on land if self.inst is stepping on the ocean overhang part of the land
        self.walk_instead = walk_instead
        self.safe_point_fn = safe_point_fn
    end)

function MYM_RunAway:__tostring()
    return string.format("MYM_RunAway %f from: %s", self.safe_dist, tostring(self.hunter))
end

function MYM_RunAway:GetRunAngle(pt, hp, sp)
    ...
    local result_offset, result_angle, deflected = find_offset_fn(pt, angle * DEGREES, radius, 8, true, false, nil,
        allowwater_or_allowboat)                                                                            -- try avoiding walls
            ...
            if self.fix_overhang and not TheWorld.Map:IsAboveGroundAtPoint(pt:Get()) then
                if self.inst.components.locomotor:IsAquatic() then
                    ...
                    if back_on_ocean ~= nil then
                        result_offset, result_angle, deflected = FindSwimmableOffset(back_on_ocean,
                            math.random() * 2 * math.pi, radius - 1, 8, true, true)
                    end
                else
                    local back_on_ground = FindNearbyLand(pt, 1) -- find a point back on proper ground
                    if back_on_ground ~= nil then
                        result_offset, result_angle, deflected = FindWalkableOffset(back_on_ground,
                            math.random() * 2 * math.pi, radius - 1, 8, true, true) -- ok don't try to avoid walls, but at least avoid water
                    end
                end
            end
            if result_angle == nil then
            ...
            if self.runshomewhenchased and ShouldGoHome(self.inst) then
                ...
                if angle ~= nil then
                    if self.walk_instead then
                        ...
                        self.inst.Transform:SetRotation(angle) --### 玩家需要先转向一下，不然第一次调用甚至往敌人脸上跑
                ...
                local safeDist = type(self.safe_dist) == "function"
                    and self.safe_dist(self.inst, self.hunter)
                    or self.safe_dist
                if distsq(hp, pt) > safeDist * safeDist then
                    self.status = SUCCESS
                    self.inst.components.locomotor:Stop()
                end
            end
