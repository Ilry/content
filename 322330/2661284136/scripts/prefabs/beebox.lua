...
local function onharvest(inst, picker, produce)
    ...
    if not inst:HasTag("burnt") then
        ...
        if inst.components.childspawner ~= nil and
            not TheWorld.state.iswinter and
            not (picker ~= nil and picker.components.skilltreeupdater ~= nil and picker.components.skilltreeupdater:IsActivated("wormwood_bugs")) and
            not (picker ~= nil and picker:HasTag("sanfenredu"))
        then
