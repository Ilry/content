...
local assets =
{
    Asset("ANIM", "anim/kyno_driftwood_pighouse.zip"),
    ...
    elseif inst.Light:IsEnabled() and inst.AnimState:IsCurrentAnimation("lit") then
        ...
        if build_name ~= inst._last_skin_build then
            ...
            if build_name ~= "" then
                ...
                snow.AnimState:SetBuild("kyno_driftwood_pighouse")
            end
        end
...
local function onstartdaylighttask(inst)
    if inst:IsLightGreaterThan(0.8) then
        LightsOff(inst)
...
local function MakeWindow()
    ...
    inst.AnimState:SetBuild("kyno_driftwood_pighouse")
...
local function fn()
	...
	minimap:SetIcon("kyno_driftwood_pighouse.tex")
