...
YELLOWSTAFF_STAR_DURATION = 999999
OPALSTAFF_STAR_DURATION = 999999
...
local function onhaunt(inst)
	...
	if inst:HasTag("hotstar") then
		SpawnPrefab("yellowstaff").Transform:SetPosition(inst.Transform:GetWorldPosition())
	else
		SpawnPrefab("opalstaff").Transform:SetPosition(inst.Transform:GetWorldPosition())
	end
    return true
end
...
local function makestafflight(name, is_hot, anim, colour, idles, is_fx)
    ...
    local function fn()
        ...
        if is_fx then
            ...
            inst.components.timer:StartTimer("extinguish", is_hot and YELLOWSTAFF_STAR_DURATION or OPALSTAFF_STAR_DURATION)
		...
		if is_hot then
			inst.components.inspectable.nameoverride = "STAFFLIGHT"
		else
			inst.components.inspectable.nameoverride = "STAFFCOLDLIGHT"
		end
