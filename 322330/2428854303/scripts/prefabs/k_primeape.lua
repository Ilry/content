...
SetSharedLootTable('kyno_primeape',
{
    {'smallmeat',     1.0},
    {'cave_banana',   1.0},
    {'beardhair',     1.0},
...
local function OnGetItemFromPlayer(inst, giver, item)
    ...
    if item.prefab == "cave_banana" then
        ...
        elseif giver.components.leader then
            ...
            inst.components.follower:AddLoyaltyTime(item.components.edible:GetHunger() * LOYALTY_TIME)
        end
        if inst.components.sleeper:IsAsleep() then
...
local function fn()
    ...
    -- inst.components.follower.maxfollowtime = 3000
