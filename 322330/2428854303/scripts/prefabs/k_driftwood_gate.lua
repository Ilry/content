    ...
    if inst.anims.narrow then
        ...
        if is_narrow then
            ...
            if not inst.bank_wide_set then
                ...
                GetAnimState(inst):SetBank("fence_gate")
            end
        end
...
local function MakeWall(name, anims, isdoor, klaussackkeyid)
    ...
    if isdoor then
        ...
        assets =
        {
			...
			Asset("ANIM", "anim/fence_gate_thin.zip"),
			Asset("ANIM", "anim/kyno_driftwood_gate.zip"),
    ...
    local function fn()
        ...
        if isdoor then
            ...
            inst.AnimState:SetBuild("kyno_driftwood_gate")
