...
local function MakeWall(name, anims, isdoor, klaussackkeyid)
    ...
    local function fn()
		...
		inst.components.inspectable.nameoverride = "FENCE_JUNK"
        ...
        inst.components.lootdropper:SetLoot({"twigs", "wagpunk_bits"})
...
local function MakeInvItem(name, placement, animdata, isdoor)
    local assets =
    {
		...
		Asset("ANIM", "anim/fence_junk.zip"),
		Asset("ANIM", "anim/fence_junk_build1.zip"),
    ...
    local function itemfn()
        ...
        inst.AnimState:SetBank("fence_junk")
        inst.AnimState:SetBuild("fence_junk_build1")
