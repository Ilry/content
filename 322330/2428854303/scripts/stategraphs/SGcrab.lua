...
local function PlayCrabFootstep(inst)
    ...
    if sound then
		sound:PlaySound("volcano/creatures/crab/walk_dirt")
    end
end
...
local function PlayCrabFootstepRun(inst)
	local sound = inst.SoundEmitter
    if sound then
		sound:PlaySound("volcano/creatures/crab/run_dirt")
    end
end
...
local states=
{
    ...
    State{
        ...
        onenter = function(inst) 
            ...
            if play_scream then
                inst.SoundEmitter:PlaySound("dontstarve_DLC002/creatures/crab/scream")
            end
