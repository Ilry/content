...
--DateCreated: 5/20/2021 3:21:02 PM
	...
	for i = 0, (iW * iH) - 1, 1 do
		...
		if (plot) then
			if (IsValidCoastalLowland(plot)) then
			    ...
			    -- An adjacent volcano or lake is also bad news  --> An adjacent volcano, lower to -500
					...
					for direction = 0, DirectionTypes.NUM_DIRECTION_TYPES - 1, 1 do
						...
						if (adjacentPlot ~= nil) then
							...
							if (adjacentPlot:IsWater()) then
								...
								--> NaturalWonder will be drowned, too
...
function MarkCoastalLowlands()
	...
	-- local numDesiredCoastalLowlandsPercentage = 100;
	--> was : local numDesiredCoastalLowlandsPercentage = GlobalParameters.CLIMATE_CHANGE_PERCENT_COASTAL_LOWLANDS or 35;

	scoredTiles = ScoreCoastalLowlandTiles();
	tilesToMark = #scoredTiles;
	--> was : tilesToMark = math.floor((#scoredTiles * numDesiredCoastalLowlandsPercentage) / 100);
	...
	if tilesToMark > 0 then
		...
		print("  " .. tostring(GlobalParameters.CLIMATE_CHANGE_PERCENT_COASTAL_LOWLANDS) .. "% of eligible coastal tiles, by Konomi's Submerging All Land");
	end
end