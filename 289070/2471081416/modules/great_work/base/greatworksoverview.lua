...
-- Adriaman: for theming
-- Real Great Musicians adds Concert Hall (Great Music) theming.
local IsRealGreatMusiciansActive = Modding.IsModActive("5d8cfe2e-7635-434a-88d9-f709a55b7544"); 

-- The first listed mod is "Wondrous Wonders (World Wonders Only)" and the second "Wondrous Wonders (Gathering Storm)"
-- Wondrous Wonders adds various Great Writing theming
local IsWondrousWondersActive = Modding.IsModActive("f9f26542-96b9-4476-827b-501a593fb03d") or Modding.IsModActive("ffc32d00-b546-4b1c-8be8-6c5d51ef1544");

-- Unearthed access differently textures.
local IsUnearthedActive = Modding.IsModActive("35a85dfb-d33a-4382-be10-a5f53c1baad1"); 
...
function UpdateGreatWorks()
	...
	local numBuildings = #cui_tmpBuildings
	local screenWidth, screenHeight = UIManager:GetScreenSizeVal();
	
	local minColumns = 4
	local maxColumns = math.floor((screenWidth-20)/255)
	
	local minRows = 4
	local maxRows = math.floor((screenHeight-155)/156)
	
	local targetColumns = minColumns
	local targetRows = minColumns
	
	-- First stretch horizontally, starting with 4 rows, then vertically
	local neededColumns = math.ceil(numBuildings / 4)
	targetColumns = math.max(minColumns, math.min(neededColumns, maxColumns))
	
	while (targetRows * targetColumns < numBuildings) and (targetRows < maxRows) do
		targetRows = targetRows + 1
	end
		
	local targetSizeX = targetColumns*255+10
	local targetSizeY = 155 + 156 * targetRows
	
	-- Manual adjustment
	Controls.PopupContainer:SetSizeX(targetSizeX)
	Controls.PopupContainer:SetSizeY(targetSizeY)
	
	Controls.LabelDisplayTitle:SetOffsetX(-targetSizeX + 100)
	Controls.LabelDisplaySpaces:SetOffsetX(-targetSizeX + 100)
	Controls.NumGreatWorks:SetOffsetX(targetSizeX - 90)
	Controls.NumDisplaySpaces:SetOffsetX(targetSizeX - 90)
	
	Controls.GreatWorksStack:SetWrapWidth(156 * targetRows)
	
	Controls.ModalBG:SetSizeY(math.min(760, targetSizeY))
	
	if targetSizeY < 750 then
		Controls.ModalBG2.SetHide(true)
	else
		Controls.ModalBG2:SetHide(false)
		Controls.ModalBG2:SetSizeY(targetSizeY-750)
	end
...
function OnClickGreatWork(kDragStruct:table, pCityBldgs:table, buildingIndex:number, greatWorkIndex:number, slotIndex:number)
	...
	local iconWidth = Controls.PlacingIcon:GetSizeX()
	local textWidth = Controls.PlacingName:GetSizeX()
	Controls.PlacingIcon:SetOffsetX(Controls.PlacingContainer:GetSizeX()/2 - textWidth/2 - iconWidth/2 - 3)
	Controls.PlacingName:SetOffsetX(Controls.PlacingContainer:GetSizeX()/2 - textWidth/2 + iconWidth/2 + 3)
...
function CuiSetThemeMask(instance, srcBldgs, srcBuilding, srcSlot)
    ...
    elseif srcGreatWorkObjectType == "GREATWORKOBJECT_MUSIC" then
		-- Adriaman: Real Great Musicians adds Concert Hall (Great Music) theming.
		if IsRealGreatMusiciansActive then
			instance.ThemeColor:SetColorByName("COLOR_FLOAT_MILITARY")
		end
    elseif srcGreatWorkObjectType == "GREATWORKOBJECT_WRITING" then
		-- Adriaman: Wondrous Wonders adds various Great Writing theming in amphitheaters and various wonders.
		if IsWondrousWondersActive then
			instance.ThemeColor:SetColorByName("COLOR_FLOAT_SCIENCE")
		end
    else
