...
m_QueuedRefreshTime = 2
m_QueuedRefreshPlacement = false
...
function QueueRefresh(withPlacement)
	 m_QueuedRefresh = true
	 m_QueuedRefreshTime = 1
	 m_QueuedRefreshPlacement = withPlacement
	 print(m_QueuedRefreshPlacement)
end

function EmptyQueueRefresh()
	 m_QueuedRefresh = false
	 m_QueuedRefreshTime = 0
	 m_QueuedRefreshPlacement = false
end
...
function KeyHandler( key:number )
	if (key == Keys.VK_ESCAPE) then
		...
		return true;
	elseif (key == Keys.VK_CONTROL) then
		m_holdingCtrl = false
		
		if m_QueuedRefresh and m_QueuedRefreshPlacement then
			RefreshQueue()
			Refresh()
			Controls.CurrentProductionContainer:SetHide(not m_hasProductionToShow);
			Controls.NoProductionContainer:SetHide(m_hasProductionToShow);
...
function KeyHandlerDown( key:number )
	if (key == Keys.VK_CONTROL) then
		m_holdingCtrl = true
		if m_QueuedRefresh and not m_QueuedRefreshPlacement then
			-- This is a dumb but the game needs to go over the loop at least twice to update :/
			if m_QueuedRefreshTime > 0 then
				m_QueuedRefreshTime = m_QueuedRefreshTime - 1
			else
				RefreshQueue()
				Refresh()
				Controls.CurrentProductionContainer:SetHide(not m_hasProductionToShow);
...
function OnInputHandler( pInputStruct:table )
	...
	elseif uiMsg == KeyEvents.KeyDown then
		return KeyHandlerDown( pInputStruct:GetKey() );
	end;
	return false;
end
