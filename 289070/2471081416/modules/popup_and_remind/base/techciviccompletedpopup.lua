...
local isMPPopUpActive = Modding.IsModActive("2b492c2e-cb99-43b0-92fa-cee943d8a28d"); -- Multiplayer Pop-Ups Enabled
...
-- ===========================================================================
--	Made by Steffalon -> Use this function only in game configuration MP
-- ===========================================================================
if isMPPopUpActive then
	function OnNotificationAdded(playerID:number, notificationID:number)
		if (playerID == Game.GetLocalPlayer()) then
			...
			local message = pNotification:GetTypeName();

			local civicIndex = pNotification:GetValue("CivicIndex");
			local techIndex = pNotification:GetValue("TechIndex");

			if civicIndex ~= nil and message == "NOTIFICATION_CIVIC_DISCOVERED" then
				AddCompletedPopup(playerID, civicIndex, nil, true);
			end

			if techIndex ~= nil and message == "NOTIFICATION_TECH_DISCOVERED" then
				AddCompletedPopup(playerID, nil, techIndex, true);
			end
...
function Initialize()
	...
	if isMPPopUpActive then
		if GameConfiguration.IsNetworkMultiplayer() then -- Must be only active for MP
			-- Check if we need to add a popup for civic or tech
