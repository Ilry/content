...
function UpdateRelationShipTooltip(tControl, playerID, allianceData)
	...
	-- Compatibility with What Did I Promise?
	if ExposedMembers.AKWDIP_GetPromisesTTString then 
		local promisett = ExposedMembers.AKWDIP_GetPromisesTTString(playerID, Game.GetLocalPlayer())
		if #promisett > 0 then 
			promisett = "[NEWLINE]" .. promisett
		end
		
		states = states .. promisett
	end
	--ak
