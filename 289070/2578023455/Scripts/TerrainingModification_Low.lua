...
local ResourceWgtIdx = 0.2;	

local additionFeatureChance = 0.05;
local rsDiscov_BaseRate = 10;
local rsDiscov_PoolWeights = rsDiscov_BaseRate;
local rsDiscov_PoolWealth = 5;
local rsDiscov_PoolCharges = 2;
local rsDiscov_PoolChargeRate = 10;
local rsDiscov_PoolChargeCounter = 0;
local rsDiscov_PoolSize = 250;
...
function reforgePlotTerrain(p_x, p_y, newTerrain, rsdisw)
	...
	local p_resource = -1;
	if p_feature == -1 then
		-- resource and feature are mutually exclusive
...
local function Initialize()
	print( " ################ Start Initializing yugongyishan Gameplay Script (Rate Low)... ################ " );
	...
	print( " ################ End Initializing yugongyishan Gameplay Script	(Rate Low)... ################ " );
end
Initialize();