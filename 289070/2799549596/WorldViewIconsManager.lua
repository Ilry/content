...
function SetResourceIcon( pInstance:table, pPlot, type, state)
	...
	if (pPlot and resourceInfo ~= nil) then
		...
		if (textureSheet ~= nil) then						
			...
			local ResourceClassTypeKey = false;
			...
			for row in GameInfo.Resource_YieldChanges() do
		        if(row.ResourceType == resourceType and row.YieldChange~=nil and row.YieldChange>0) then
			        local str = "[ICON"..string.sub(row.YieldType,6).."]";
			        table.insert(toolTipItems,str .. " + "..row.YieldChange);
		        end
		    end
			if (resourceTechType ~= nil and resourceType~="RESOURCE_ANTIQUITY_SITE" and resourceType~="RESOURCE_SHIPWRECK") then
				...
				if (localPlayer ~= nil) then
					...
					if (techType ~= nil and not playerTechs:HasTech(techType.Index)) then
						table.insert(toolTipItems,"[NEWLINE][COLOR:Civ6Red]"..Locale.Lookup("LOC_UNITOPERATION_BUILD_IMPROVEMENT_DESCRIPTION")..Locale.Lookup("LOC_TOOLTIP_BUILDING_REQUIRES").." " .. Locale.Lookup(techType.Name) .. "[ENDCOLOR]");
					end
				end
			end

			if (resourceCivicType ~= nil and resourceType~="RESOURCE_ANTIQUITY_SITE" and resourceType~="RESOURCE_SHIPWRECK") then
				local localPlayer	= Players[Game.GetLocalPlayer()];
				if (localPlayer ~= nil) then
					...
					if (civicType ~= nil and not playerCulture:HasCivic(civicType.Index)) then
						table.insert(toolTipItems,"[NEWLINE][COLOR:Civ6Red]"..Locale.Lookup("LOC_UNITOPERATION_BUILD_IMPROVEMENT_DESCRIPTION")..Locale.Lookup("LOC_TOOLTIP_BUILDING_REQUIRES").." ".. Locale.Lookup(civicType.Name) .. "[ENDCOLOR]");
					end
				end
			end
      ...
      local key=true;
		    for row in GameInfo.Resource_Harvests() do
		    	if(row.ResourceType == resourceType and row.YieldType~=nil and row.YieldType~="" and row.Amount>0 and resourceType~="RESOURCE_ANTIQUITY_SITE" and resourceType~="RESOURCE_SHIPWRECK") then
			        local str = "[ICON"..string.sub(row.YieldType,6).."]";
			         if row.PrereqTech ~=nil and row.PrereqTech ~="" and key then
			        table.insert(toolTipItems, "[NEWLINE][COLOR:Civ6Red]"..Locale.Lookup("LOC_UNITOPERATION_HARVEST_RESOURCE_DESCRIPTION")..Locale.Lookup("LOC_TOOLTIP_BUILDING_REQUIRES").." "..Locale.Lookup("LOC_"..row.PrereqTech.."_NAME").." [ENDCOLOR]");
			        key=false;
			      elseif key then
			        key=false;
			      end
			        table.insert(toolTipItems,Locale.Lookup("LOC_UNITOPERATION_HARVEST_RESOURCE_DESCRIPTION")..Locale.Lookup("LOC_CITY_STATES_BONUSES_EARNED")..str);
		        end
		    end

		    if ResourceClassTypeKey and GameInfo.ResourceIndustries~=nil and GameInfo.ResourceIndustries~="" then 
		    	for row in GameInfo.ResourceIndustries() do
                   if(row.ResourceType == resourceType and row.ResourceEffectTExt~=nil and row.ResourceEffectTExt~="" and resourceType~="RESOURCE_ANTIQUITY_SITE" and resourceType~="RESOURCE_SHIPWRECK") then
			        table.insert(toolTipItems,"[NEWLINE][COLOR:Civ6Red]"..Locale.Lookup("LOC_IMPROVEMENT_INDUSTRY_TYPE_NAME","LOC_"..row.ResourceType.."_NAME").."[ENDCOLOR] "..Locale.Lookup(row.ResourceEffectTExt));
		           end
		    	end
		    end
