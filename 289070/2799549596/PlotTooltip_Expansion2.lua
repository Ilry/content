...
function GetDetails(data)
	...
	if(data.WonderType ~= nil) then
		...
		if (data.WonderComplete == true) then
			...
			table.insert(details, "");
        local Modstr="";
        for i,pPlayer in pairs(PlayerManager.GetAliveMajors()) do
		    ...
		    for i, pCity in pPlayer:GetCities():Members() do
				...
				if (cityBuildHash ~= nil) then
					if (GameInfo.Buildings[cityBuildHash] == GameInfo.Buildings[data.WonderType]) then
                        local pCivilizationName = "LOC_"..pPlayerConfig:GetCivilizationTypeName().."_NAME";
                        local CivilizationName = "[ICON_Production]"..Locale.Lookup(pCivilizationName).."还剩";
                        local TurnsLeft = ""..pCity:GetBuildQueue():GetTurnsLeft().."[ICON_turn] 建造完成";
                        local Modstr=CivilizationName..TurnsLeft;
                        table.insert(details,Modstr); 
                    end
			    end
		    end	
	    end
	...
	if(data.IsCity == true and data.DistrictType ~= nil) then
		...
		table.insert(details, "-------------------------------");
		if Players[data.Owner]:GetCities()~= nil then

		for i,pCity in Players[data.Owner]:GetCities():Members() do
			if pCity:GetName()==data.OwningCityName then
				local culturePerTurn=Locale.Lookup(GameInfo.Yields['YIELD_CULTURE'].IconString) .." "..(math.floor( pCity:GetYield( YieldTypes.CULTURE )*10) / 10);
	            local faithPerTurn=Locale.Lookup(GameInfo.Yields['YIELD_FAITH'].IconString).. " "..(math.floor( pCity:GetYield( YieldTypes.FAITH )*10) / 10);
	            local foodPerTurn=Locale.Lookup(GameInfo.Yields['YIELD_FOOD'].IconString).." "..(math.floor( pCity:GetYield( YieldTypes.FOOD )*10) / 10);
	            local goldPerTurn=Locale.Lookup(GameInfo.Yields['YIELD_GOLD'].IconString).." "..(math.floor( pCity:GetYield( YieldTypes.GOLD )*10) / 10);
	            local productionPerTurn=Locale.Lookup(GameInfo.Yields['YIELD_PRODUCTION'].IconString).." "..(math.floor( pCity:GetYield( YieldTypes.PRODUCTION )*10) / 10);
	            local sciencePerTurn=Locale.Lookup(GameInfo.Yields['YIELD_SCIENCE'].IconString).." "..(math.floor( pCity:GetYield( YieldTypes.SCIENCE )*10) / 10);
                table.insert(details,foodPerTurn.." "..productionPerTurn.." "..goldPerTurn);
                table.insert(details,sciencePerTurn.." "..culturePerTurn.." "..faithPerTurn);
                local key=false;
                local pPlayers=PlayerManager.GetAliveMajors();
                for i,pPlayer in ipairs(pPlayers) do
                	for _,pUnit in pPlayer:GetUnits():Members() do
                	local plot = Map.GetPlot(pUnit:GetX(),pUnit:GetY());
                    if GameInfo.Units[pUnit:GetType()].UnitType == "UNIT_SPY" and Cities.GetPlotPurchaseCity(plot)==pCity and pUnit:GetOwner()~=data.Owner then 
                        table.insert(details,Locale.Lookup("[ICON_Amenities]")..pCity:GetGrowth():GetHappiness().."    "..Locale.Lookup("[ICON_Housing]")..pCity:GetGrowth():GetHousing().."    [COLOR_RED]"..Locale.Lookup("[ICON_Governor]")..Locale.Lookup("LOC_ESPIONAGEOVERVIEW_OPERATIVES_HEADER").."[ENDCOLOR]");
                        key=true;
                    end
                    end
                end
                if key==false then table.insert(details,Locale.Lookup("[ICON_Amenities]")..pCity:GetGrowth():GetHappiness().."    "..Locale.Lookup("[ICON_Housing]")..pCity:GetGrowth():GetHousing().."    [COLOR_GREEN]"..Locale.Lookup("[ICON_Governor]")..Locale.Lookup("LOC_ESPIONAGEOVERVIEW_OPERATIVES_HEADER").."[ENDCOLOR]") end
				break;
			end
		end
	end
		...
		if (not GameInfo.Districts[data.DistrictType].InternalOnly) then	--Ignore 'Wonder' districts
			...
			if (data.DistrictYields ~= nil) then
				for yieldType, v in pairs(data.DistrictYields) do
					local yield = GameInfo.Yields[yieldType].Name;
					local yieldicon = GameInfo.Yields[yieldType].IconString;
					strDis = Locale.Lookup(yieldicon) .. tostring(v);
					table.insert(details, str);
				end
			end
      if #strDis<20 then
		table.insert(details,sDistrictName.." "..strDis);
	else
   table.insert(details,sDistrictName);
   table.insert(details, "  "..strDis);
	end
			if(data.ResourceType ~= nil and data.DistrictType ~= nil) then
				local localPlayer = Players[Game.GetLocalPlayer()];
				if (localPlayer ~= nil) then
			...
			for i, v in ipairs(data.BuildingNames) do 
			    if (data.WonderType == nil) then
					if (data.BuildingsPillaged[i]) then
						local str	="";
		local building = GameInfo.Buildings[data.BuildingTypes[i]];
		local buildingName = building.Name;
		if v==buildingName then
		  for yieldRow in GameInfo.Yields() do
			  local yieldChange = data.OwnerCity:GetBuildingYield(data.BuildingTypes[i], yieldRow.YieldType);
			  if yieldChange ~= 0 then
				  str=str..Locale.Lookup(GameInfo.Yields[yieldRow.YieldType].IconString)..yieldChange.." ";
			  end
		  end
						end
						if #str<20 then
		table.insert(details, "-" .. Locale.Lookup(v) .. " " .. Locale.Lookup("LOC_TOOLTIP_PLOT_PILLAGED_TEXT").." "..str);
	else
   ...
   table.insert(details, "  "..str);
		...
		if #str<20 then
		table.insert(details, "-" .. Locale.Lookup(v).." "..str);
	else
   table.insert(details, "-" .. Locale.Lookup(v));
   table.insert(details, "  "..str);
	end
