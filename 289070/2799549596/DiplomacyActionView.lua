...
-- Obtain "uniques" from Civilization and for the chosen leader
	...
	local count=0;
	for i,v in ipairs(uniqueAbilities) do
		if v~=nil then
        count=count+1;
        end
	end
	...
	for _, item in ipairs(uniqueAbilities) do
		if item.Name and item.Name ~= "NONE"  then
			...
			if count==_ then
			instance.Description:SetText( Locale.Lookup( item.Description ) .."[NEWLINE][NEWLINE]");
		    else
	...
	for _, item in ipairs(uniqueUnits) do
		...
		local headerText:string = Locale.ToUpper(Locale.Lookup( item.Name ));
		local str="";
            if GameInfo.Units[item.Type].PrereqCivic ~= nil and GameInfo.Units[item.Type].PrereqCivic ~= "" then 
            	str=str..Locale.Lookup(''..GameInfo.Eras[''..GameInfo.Civics[GameInfo.Units[item.Type].PrereqCivic].EraType].Name).." "..Locale.Lookup('LOC_CIVIC_NAME').."  ";
             	str='[COLOR_FLOAT_CULTURE]'..str..Locale.Lookup(''..GameInfo.Civics[GameInfo.Units[item.Type].PrereqCivic].Name)..'[ENDCOLOR]';
            end
            if GameInfo.Units[item.Type].PrereqTech ~= nil and GameInfo.Units[item.Type].PrereqTech ~= "" then 
            	str=str..Locale.Lookup(''..GameInfo.Eras[''..GameInfo.Technologies[GameInfo.Units[item.Type].PrereqTech].EraType].Name).." "..Locale.Lookup('LOC_TECHNOLOGY_NAME').."  ";
             	str='[COLOR_FLOAT_SCIENCE]'..str..Locale.Lookup(''..GameInfo.Technologies[GameInfo.Units[item.Type].PrereqTech].Name)..'[ENDCOLOR]'; 
            end
            if str=="" then
                instance.Header:SetText( headerText..'    [COLOR_RED]'..Locale.Lookup("LOC_CIVICS_KEY_AVAILABLE")..'[ENDCOLOR]');
            else
            	instance.Header:SetText( headerText..'    '..str);
            end
			if string.sub(Locale.Lookup('LOC_'..item.Type..'_DESCRIPTION'),1,3)=="LOC" then
			    instance.Description:SetText(Locale.Lookup('LOC_'..item.Type..'_EXPANSION2_DESCRIPTION'));
		    else		    
			    instance.Description:SetText(Locale.Lookup('LOC_'..item.Type..'_DESCRIPTION')..'[NEWLINE]');
			end
	end
	...
	for _, item in ipairs(uniqueBuildings) do
		...
		local headerText:string = Locale.ToUpper(Locale.Lookup( item.Name ));
		local str="";
			local itemInfo = GameInfo.Buildings[item.Type] or GameInfo.Districts[item.Type] or GameInfo.Improvements[item.Type]; 
            if itemInfo.PrereqCivic ~= nil and itemInfo.PrereqCivic ~= "" then 
            	str=str..Locale.Lookup(''..GameInfo.Eras[''..GameInfo.Civics[itemInfo.PrereqCivic].EraType].Name).." "..Locale.Lookup('LOC_CIVIC_NAME').."  ";
             	str='[COLOR_FLOAT_CULTURE]'..str..Locale.Lookup(''..GameInfo.Civics[itemInfo.PrereqCivic].Name)..'[ENDCOLOR]';
            end
            if itemInfo.PrereqTech ~= nil and itemInfo.PrereqTech ~= "" then 
            	str=str..Locale.Lookup(''..GameInfo.Eras[''..GameInfo.Technologies[itemInfo.PrereqTech].EraType].Name).." "..Locale.Lookup('LOC_TECHNOLOGY_NAME').."  ";
             	str='[COLOR_FLOAT_SCIENCE]'..str..Locale.Lookup(''..GameInfo.Technologies[itemInfo.PrereqTech].Name)..'[ENDCOLOR]'; 
            end
            if str=="" then
                instance.Header:SetText( headerText..'    [COLOR_RED]'..Locale.Lookup("LOC_CIVICS_KEY_AVAILABLE")..'[ENDCOLOR]');
            else
            	instance.Header:SetText( headerText..'    [COLOR_RED]'..str..'[ENDCOLOR]');
            end
			if string.sub(Locale.Lookup('LOC_'..item.Type..'_DESCRIPTION'),1,3)=="LOC" then
			    instance.Description:SetText(Locale.Lookup('LOC_'..item.Type..'_EXPANSION2_DESCRIPTION'));
		    else		    
