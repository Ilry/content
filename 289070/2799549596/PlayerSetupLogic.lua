...
function SetUniqueCivLeaderData(info:table, tooltipControls:table)
	...
	if (info.CivilizationAbility) then
		...
		civAbility.Description:SetText(Locale.Lookup(info.CivilizationAbility.Description).."[NEWLINE]");
		hasTooltipInfo = true;
	end
	...
	if (info.Uniques) then
		for _, item in ipairs(info.Uniques) do
			...
			local str=string.sub(item.Icon, 6,6);
			local headerText:string = Locale.ToUpper(Locale.Lookup( item.Name ));
			if str=="U" then   
				instance.Header:SetText('[COLOR_FLOAT_MILITARY]'..headerText..'[ENDCOLOR]');
			elseif str=="B" then  
				instance.Header:SetText('[COLOR_FLOAT_GOLD]'..headerText..'[ENDCOLOR]');
			elseif str=="D" then  
				instance.Header:SetText('[COLOR_FLOAT_SCIENCE]'..headerText..'[ENDCOLOR]');
			elseif str=="I" then
                instance.Header:SetText('[COLOR_FLOAT_FOOD]'..headerText..'[ENDCOLOR]');
			else
                instance.Header:SetText(headerText);
			end
			instance.Description:SetText(Locale.Lookup(item.Description).."[NEWLINE]");
			hasTooltipInfo = true;
		end
	end
