		    ...
		    for i, pCity in pPlayer:GetCities():Members() do
				...
				if (cityBuildHash ~= nil) then
					if (GameInfo.Buildings[cityBuildHash] == GameInfo.Buildings[data.WonderType]) then
                        ...
                        local CivilizationName = "[ICON_Production]"..Locale.Lookup(pCivilizationName).." ";
                        local CityName =Locale.Lookup(pCityName).." "..Locale.Lookup("LOC_TURNS_REMAINING_VAL",pCity:GetBuildQueue():GetTurnsLeft().."[ICON_turn]");
                        local Modstr=CivilizationName..CityName;
	...
	if(data.IsCity == true and data.DistrictType ~= nil) then
		...
		for i,pCity in Players[data.Owner]:GetCities():Members() do
			if pCity:GetName()==data.OwningCityName then
				local culturePerTurn=Locale.Lookup("LOC_TOP_PANEL_CULTURE").." "..Locale.Lookup(GameInfo.Yields['YIELD_CULTURE'].IconString) .." "..(math.floor( pCity:GetYield( YieldTypes.CULTURE )*100) / 100);
	            local faithPerTurn=Locale.Lookup("LOC_TOP_PANEL_FAITH").." "..Locale.Lookup(GameInfo.Yields['YIELD_FAITH'].IconString).. " "..(math.floor( pCity:GetYield( YieldTypes.FAITH )*100) / 100);
	            local foodPerTurn=Locale.Lookup("LOC_TECH_FILTER_FOOD").." "..Locale.Lookup(GameInfo.Yields['YIELD_FOOD'].IconString).." "..(math.floor( pCity:GetYield( YieldTypes.FOOD )*100) / 100);
	            local goldPerTurn=Locale.Lookup("LOC_TOP_PANEL_GOLD").." "..Locale.Lookup(GameInfo.Yields['YIELD_GOLD'].IconString).." "..(math.floor( pCity:GetYield( YieldTypes.GOLD )*100) / 100);
	            local productionPerTurn=Locale.Lookup("LOC_TECH_FILTER_PRODUCTION").." "..Locale.Lookup(GameInfo.Yields['YIELD_PRODUCTION'].IconString).." "..(math.floor( pCity:GetYield( YieldTypes.PRODUCTION )*100) / 100);
	            local sciencePerTurn=Locale.Lookup("LOC_TOP_PANEL_SCIENCE").." "..Locale.Lookup(GameInfo.Yields['YIELD_SCIENCE'].IconString).." "..(math.floor( pCity:GetYield( YieldTypes.SCIENCE )*100) / 100);
                table.insert(details,foodPerTurn);
                table.insert(details,productionPerTurn);
                table.insert(details,sciencePerTurn);
                table.insert(details,culturePerTurn);
                table.insert(details,goldPerTurn);
                table.insert(details,faithPerTurn);
                table.insert(details,Locale.Lookup("LOC_TECH_FILTER_AMENITIES").." "..Locale.Lookup("[ICON_Amenities]")..pCity:GetGrowth():GetHappiness());
                table.insert(details,Locale.Lookup("LOC_TECH_FILTER_HOUSING").." "..Locale.Lookup("[ICON_Housing]")..pCity:GetGrowth():GetHousing());
				break;
			end
		end
	end
