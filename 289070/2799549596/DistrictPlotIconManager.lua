  ...
  while i<=y+range do
  	while j<=x+range do
  		if i>=0 and i<=height then
  			z = j;
  			if j<0 then z=width-j; end
  			if j>width then z=j-width; end
  			if Map.GetPlot(z,i):GetOwner()==-1 or Map.GetPlot(z,i):GetOwner()==pCity:GetOwner() then
  			if Map.GetPlot(z,i):GetDistrictType()==-1 and Map.GetPlot(z,i):IsMountain()==false and Map.GetPlot(z,i):IsNaturalWonder()==false then
  			    	if Map.GetPlot(z,i):GetResourceType()==-1 then
  			    	table.insert(plots,Map.GetPlot(z,i):GetIndex());
  			    else
  			    	for row in GameInfo.Resource_Harvests() do
		if(row.ResourceType == GameInfo.Resources[Map.GetPlot(z,i):GetResourceType()].ResourceType) then
			table.insert(plots,Map.GetPlot(z,i):GetIndex());
		end
	end
  			   end 					
