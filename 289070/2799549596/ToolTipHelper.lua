...
---------------------------------------------------core----------------------------------------------------------  
if GameInfo.Buildings[buildingHash].IsWonder then
    local Modkey=false;
	...
	if (tResults[CityCommandResults.PLOTS] ~= nil and table.count(tResults[CityCommandResults.PLOTS]) ~= 0) then
		...
		for i,plotID in pairs(plots) do
			local kPlot:table =  Map.GetPlotByIndex(plotID);
				if kPlot:CanHaveWonder(GameInfo.Buildings[buildingHash].Index, pCity:GetOwner(), pCity:GetID()) then
					if Modkey==false then 
            table.insert(toolTipLines,"");
					end
					table.insert(toolTipLines,"[ICON_Production]"..Locale.Lookup(pCity:GetName()));
					Modkey=true;
					break;
				end
		end
    ...
    for i,pPlayer in pairs(PlayerManager.GetAliveMajors()) do
		    ...
		    for i, pCity in pPlayer:GetCities():Members() do
				...
				if (cityBuildHash ~= nil) then
					if (cityBuildHash == buildingHash) then
            ...
            table.insert(toolTipLines,"");
					...
					table.insert(toolTipLines,Modstr); 
					Modkey=true;
				end
                    end
			    end
