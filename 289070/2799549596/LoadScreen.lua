...
function OnLoadScreenContentReady()
	...
	if playerConfig == nil then
		...
		for _, item in ipairs(uniqueUnits) do
            ...
            local des=Locale.Lookup('LOC_'..item.Type..'_DESCRIPTION');
			if string.sub(des,1,3)=="LOC" then
			    des=Locale.Lookup('LOC_'..item.Type..'_EXPANSION2_DESCRIPTION');
			end
		    if string.sub(des,1,3)=="LOC" then	    
			    des=Locale.Lookup(item.Description);
			end
			instance.Description:SetText(des);
		end
