...
------------------------------------------------------------------
function ShowTopData(playerID,result)
	local hasWonder=false;
  for building in GameInfo.Buildings() do
  	if Players[playerID]:GetCities()==nil then break end
		for i,pCity in Players[playerID]:GetCities():Members() do
			if (pCity:GetBuildings():HasBuilding(building.Index) and building.RequiresPlacement) then
				local typeName = building.BuildingType;
				result = result..Locale.Lookup("LOC_"..typeName.."_NAME").."[NEWLINE]";
				hasWonder=true;
			end
		end
  end
    ...
    return result..Locale.Lookup("LOC_HOF_STAT_WONDERS").." 0 [NEWLINE]";
  end
end
------------------------------------------------------------------
	...
	if pPlayerConfig and pPlayerConfig:GetLeaderTypeName() then
		...
		if GameConfiguration.IsAnyMultiplayer() and isHuman then
			if(playerID ~= localPlayerID and not Players[localPlayerID]:GetDiplomacy():HasMet(playerID)) then
				result = Locale.Lookup("LOC_DIPLOPANEL_UNMET_PLAYER") .. " (" .. pPlayerConfig:GetPlayerName() .. ")".. "[NEWLINE][NEWLINE]";
				result = ShowTopData(playerID,result);
			else
				result = Locale.Lookup("LOC_DIPLOMACY_DEAL_PLAYER_PANEL_TITLE", leaderDesc, civDesc) .. " (" .. pPlayerConfig:GetPlayerName() .. ")".. "[NEWLINE][NEWLINE]";
				result = ShowTopData(playerID,result);
			end
		else
			if(playerID ~= localPlayerID and not Players[localPlayerID]:GetDiplomacy():HasMet(playerID)) then
				result = Locale.Lookup("LOC_DIPLOPANEL_UNMET_PLAYER").. "[NEWLINE][NEWLINE]";
				result = ShowTopData(playerID,result);
			else
				result = Locale.Lookup("LOC_DIPLOMACY_DEAL_PLAYER_PANEL_TITLE", leaderDesc, civDesc).. "[NEWLINE][NEWLINE]";
				result = ShowTopData(playerID,result);
			end
		end
	end
