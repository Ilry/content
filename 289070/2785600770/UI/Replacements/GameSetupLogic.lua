...
function GameParameters_WriteAuxParameterValues(o, parameter)
	...
	if(parameter.ParameterId == "Ruleset" and GameSetup_PlayerCountChanged) then
		GameSetup_PlayerCountChanged(m_SingleModeOn);
	end

	if parameter.ParameterId == "GameMode_Knm_Single" and GameSetup_PlayerCountChanged then
		m_SingleModeOn = parameter.Value
...
function MapSize_ValueNeedsChanging(p)
	...
	-- >>> SingleNoAIMode: in case that after changing map size, start button is disabled.
	if m_SingleModeOn then
		minPlayers = 1
		maxPlayers = 1
	end
	-- <<<
...
function MapSize_ValueChanged(p)
	...
	-- >>> SingleNoAIMode: if this mode is on, when player change map size, we force to adjust participants to only ONE.
	if m_SingleModeOn then
