...
-- ===========================================================================
function RefreshPlayerSlots(isSetPlayerOne)
	...
	-- >>> SingleNoAIMode: when player turn on this mode, we force to adjust participants to only ONE.
	if isSetPlayerOne then
		minPlayers = 1
		maxPlayers = 1
	end
	-- <<<
...
-- ===========================================================================
-- >>> SingleNoAIMode: when player turn on sunch mode, we force to adjust participants to only ONE.
local m_IsSingleModeOn = false
function GameSetup_PlayerCountChanged(isSetPlayerOne)
	print("Player Count Changed", isSetPlayerOne);
	m_IsSingleModeOn = isSetPlayerOne
	RefreshPlayerSlots(isSetPlayerOne);
end
-- <<<
...
function OnStartButton()
	...
	if (GameConfiguration.IsWorldBuilderEditor()) then
		...
		-- >>> SingleNoAIMode: if keeps 1 player, then barbarians will not appear ingame. Barbarians' creating mechanism seems 
		-- in DLL, so we CANNOT modify. 
		-- Fuck Firaxis
		print('m_IsSingleModeOn', m_IsSingleModeOn)

		if m_IsSingleModeOn then
